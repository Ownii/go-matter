...
PASE produces a working `Ke` on both sides but nothing post-handshake is encrypted yet: `crypto.HKDF` is fixed at 16 bytes and `session.SessionManager.{Encrypt,Decrypt}Payload` are pass-throughs. AES-CCM and the §5.3.1 nonce layout are now real, so the only remaining Phase 3 blocker is §11 (variable-length HKDF) before Phase 4 can wire session keys end-to-end.
